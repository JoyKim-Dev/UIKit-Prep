...
구현 실패: 최근 검색어 버튼 4개에 최신순으로 밀려나면서 텍스트 삽입하는 기능..
...
class NewTermViewController: UIViewController, UITextFieldDelegate {
        ...
        // 이전검색어 버튼 로직 함수
        func searchedTermBtnLogic() {
        

            searchedTermBtnList[0].setTitle(mainTextField.text, for: .normal)
         
            
            // 고정 자리가 아닌 최신 검색어로 버튼 밀려나는 기능 구현 실패..
    ...
    func searchText(){
      ...
      // 텍스트필드 리턴 버튼 누르면 로직 실행(레이블 로직 & 버튼 로직 실행 함수로 구현)
        func textFieldShouldReturn(_ textField: UITextField) -> Bool {
            searchText()
            designSearchedTermBtn()
            searchedTermBtnLogic()
        ...
        // 검색 버튼 누르면 로직 실행(검색 결과에 따라 레이블 표시- 조건 & 버튼 UI 변경)
